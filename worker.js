...
export default {
  async fetch(request, env) {
    ...
    try {
      ...
      // IMPORTANT: Send the real error back to Telegram so
      // Cloudflare/Gemini problems can be diagnosed from the phone.
      ...
      if (chatId) {
        try {
          ...
          console.error("ERROR_SEND_FAILED", sendErr);
        }
      }
    }
...
async function telegram(env, method, body) {
  ...
  // Log the real Telegram response for Cloudflare Observability.
  // The bot token is never included in this log.
  console.error("TELEGRAM_API_RESULT", JSON.stringify({
    method,
    status: r.status,
    ok: r.ok,
  ...
  if (!r.ok) {
    throw new Error(
      `Telegram ${method} HTTP ${r.status}: ${responseText.slice(0, 2000)}`
    );
  }
  ...
  try {
    ...
    throw new Error(
      `Telegram ${method} returned invalid JSON: ${responseText.slice(0, 1000)}`
    );
  }

  if (json.ok === false) {
    throw new Error(
      `Telegram ${method} API error: ${json.description || responseText}`
    );
  }
...
async function getTelegramFile(env, fileId) {
  ...
  const data = await fetch(
    `https://api.telegram.org/file/bot${String(env.TELEGRAM_BOT_TOKEN).trim()}/${path}`
  );
...
async function sendDocument(env, chatId, filename, content, caption) {
  ...
  const r = await fetch(
    `https://api.telegram.org/bot${String(env.TELEGRAM_BOT_TOKEN).trim()}/sendDocument`,
    {
      method: "POST",
      body: form
    }
